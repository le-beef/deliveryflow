...
function listEntities(entityType) { return database.prepare("SELECT entity_id,data,updated_at FROM network_entities WHERE entity_type=? ORDER BY updated_at DESC").all(entityType).map((row) => ({ ...JSON.parse(row.data), networkId: row.entity_id, networkUpdatedAt: row.updated_at })); }
